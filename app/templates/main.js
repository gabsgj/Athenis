...
document.addEventListener('DOMContentLoaded', () => {
  ...
  analyzeBtn.addEventListener('click', async () => {
        ...
        if (task === 'full-analysis') {
            endpoint = '/api/full-analysis';
            body = { text };
        } else {
        ...
        resultOutput.innerHTML = '<p>Analyzing...</p>';
        riskList.innerHTML = '';

        try {
            if (stream && task !== 'full-analysis') {
                ...
                if (data.ok) {
                    if (task === 'full-analysis') {
                        resultOutput.innerHTML = `
                            <h3 class="font-bold">Simplified</h3>
                            <p>${data.result.simplified}</p>
                            <h3 class="font-bold mt-4">Summary</h3>
                            <p>${data.result.summary}</p>
                        `;
                        renderRisks(data.result.risk);
                    } else {
                        resultOutput.textContent = data.result.plain_language || JSON.stringify(data.result, null, 2);
                    }
                } else {
    ...
    function renderRisks(risks) {
        if (risks && risks.length > 0) {
            risks.forEach(risk => {
                ...
                li.innerHTML = `<strong>${risk.type} (${risk.severity}):</strong> ${risk.explanation}`;
